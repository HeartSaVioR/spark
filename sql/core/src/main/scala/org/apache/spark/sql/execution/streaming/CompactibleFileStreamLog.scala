      ...
      addNewBatchByStream(batchId) { output =>
        ...
        if (metadataLogVersion == 1) {
          ...
          validBatches.foreach { id =>
            foreachInBatch(id) { entry => writeEntryToV1(entry, output) }
          }
          ...
          val validBatches = getValidBatchesBeforeCompactionBatch(batchId, compactInterval)
          validBatches.foreach { id =>
            foreachInBatch(id) { entry => writeEntryToV2(entry, dos) }
          }
  ...
  private class Version2ReadIterator(in: InputStream) extends Iterator[T] {
    ...
    private def fillEntry(): Unit = {
      ...
      if (size == -1) {
        ...
        throw new IllegalStateException(s"Error to deserialize file: size cannot be $size")
      } else {
