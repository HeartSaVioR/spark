 ...
 *
 * SQLAppStatusListenerJsonProtocolSuite contains backwards-compatibility tests which check that the current
 * version of SQLLiveEntityJsonProtocol is able to read output written by earlier versions.
...
private[sql] object SQLAppStatusListenerJsonProtocol {
  ...
  def stateToJson(listener: SQLAppStatusListener): JValue = {
    ...
    val liveExecutions = listener.liveExecutions.entrySet().asScala.map { entry =>
      entry.getKey.toString -> liveExecutionDataToJson(entry.getValue)
    }.toMap
    val stageMetrics = listener.stageMetrics.entrySet().asScala.map { entry =>
      entry.getKey.toString -> liveStageMetricsToJson(entry.getValue)
    }.toMap
  ...
  def liveExecutionDataToJson(execution: LiveExecutionData): JValue = {
    val completionTime = execution.completionTime.map(_.getTime).map(JLong(_)).getOrElse(JNothing)
    val jobs = execution.jobs.map { entry => entry._1.toString -> entry._2.name() }
    val driverAccumUpdates = execution.driverAccumUpdates.map { entry =>
      entry._1.toString -> entry._2
    }
    val metricsValues = Option(execution.metricsValues).map { m =>
      m.map { case (key, value) => key.toString -> value }
    }
  ...
  def liveStageMetricsToJson(metrics: LiveStageMetrics): JValue = {
    import scala.collection.JavaConverters._
    val taskMetrics = metrics.taskMetrics.entrySet().asScala.map { entry =>
      entry.getKey.toString -> liveTaskMetricsToJson(entry.getValue)
    }.toMap
