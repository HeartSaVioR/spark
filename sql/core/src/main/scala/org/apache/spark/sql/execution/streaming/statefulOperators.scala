...
trait StateStoreReader extends StatefulOperator {
  override lazy val metrics = Map(
    ...
    "restoreStateValueToOriginRowOverheadTimeMs" -> SQLMetrics.createMetric(sparkContext,
      "total time for overhead of restoring state value to origin row"))
}
...
trait StateStoreWriter extends StatefulOperator { self: SparkPlan =>
  ...
  override lazy val metrics = Map(
    ...
    "projectOriginRowToStateValueOverheadTimeMs" -> SQLMetrics.createMetric(sparkContext,
      "total time for overhead of projecting origin row to state value"),
    "restoreStateValueToOriginRowOverheadTimeMs" -> SQLMetrics.createMetric(sparkContext,
      "total time for overhead of restoring state value to origin row")
...
case class StateStoreRestoreExec(
  ...
  val valueExpressions: Seq[Attribute] = child.output.diff(keyExpressions)
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    child.execute().mapPartitionsWithStateStore(
      ...
      Some(sqlContext.streams.stateStoreCoordinator)) { case (store, iter) =>
        ...
        val restoreOverheadTimeMs = longMetric("restoreStateValueToOriginRowOverheadTimeMs")
        ...
        if (!hasInput && keyExpressions.isEmpty) {
          ...
          iter.flatMap { row =>
            ...
            val (restoredRow, timeMs) = Utils.timeTakenMs {
              if (savedState == null) {
                savedState
              } else {
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    child.execute().mapPartitionsWithStateStore(
      ...
      Some(sqlContext.streams.stateStoreCoordinator)) { (store, iter) =>
        ...
        val projectValueOverheadTimeMs = longMetric("projectOriginRowToStateValueOverheadTimeMs")
        val restoreOverheadTimeMs = longMetric("restoreStateValueToOriginRowOverheadTimeMs")
          ...
          case Some(Complete) =>
            allUpdatesTimeMs += timeTakenMs {
              while (iter.hasNext) {
                ...
                val (value, timeMs) = Utils.timeTakenMs {
                  // TODO: could we do better to remove overhead for "copying" inside store?
                  getValue(row)
                }
                projectValueOverheadTimeMs += timeMs
            ...
            store.iterator().map { rowPair =>
              ...
              val (retRow, timeMs) = Utils.timeTakenMs {
