 ...
 * - Store 3 ([[KeyWithIndexToMatchedStore]]) maintains mapping between (key, index) -> matched
 ...
 *          read each of the n values in KeyWithIndexToValueStore, along with KeyWithIndexToMatchedStore
 * -
 ...
 *          delete from key from KeyToNumValuesStore, delete values in KeyWithIndexToValueStore
  *         and KeyWithIndexToMatchedStore
 ...
 *          the predicate, delete corresponding (key, indexToDelete) from KeyWithIndexToValueStore
  *         and KeyWithIndexToMatchedStore by overwriting with the value of (key, maxIndex),
  *         and removing [(key, maxIndex), decrement corresponding num values in
  *         KeyToNumValuesStore and KeyWithIndexToMatchedStore
 */
class SymmetricHashJoinStateManager(
  ...
  /** Get all the (value, matched) pairs of a key */
  def get(key: UnsafeRow): Iterator[ValueAndMatched] = {
    val numValues = keyToNumValues.get(key)
    val reusePair = ValueAndMatched()
    keyWithIndexToValue.getAll(key, numValues).map { keyIdxToValue =>
      val idx = keyIdxToValue.valueIndex
      reusePair.copy(value = keyIdxToValue.value, matched = keyWithIndexToMatched.get(key, idx))
    }
  }

  def getJoiningRows(key: UnsafeRow, predicate: UnsafeRow => Boolean): Iterator[UnsafeRow] = {
    val numValues = keyToNumValues.get(key)
    keyWithIndexToValue.getAll(key, numValues).filter { keyIdxToValue =>
      if (predicate(keyIdxToValue.value)) {
  ...
  /** Mark whether the value is matched against other value in join */
  def markMatched(key: UnsafeRow, idx: Long, matched: Boolean): Unit = {
    val numExistingValues = keyToNumValues.get(key)
    require(idx < numExistingValues)
    keyWithIndexToMatched.put(key, idx, matched)
  }
  ...
  // FIXME: let me clean up this later...
  /**
...
object SymmetricHashJoinStateManager {
   ...
   * Designed for object reuse.
   */
  case class ValueAndMatched(
      var value: UnsafeRow = null, var matched: Boolean = false) {
    def withNew(newValue: UnsafeRow, newMatched: Boolean): this.type = {
      this.value = newValue
      this.matched = newMatched
      this
    }
