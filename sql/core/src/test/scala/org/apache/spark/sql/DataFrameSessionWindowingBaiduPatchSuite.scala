...
class DataFrameSessionWindowingBaiduPatchSuite
  ...
  test("session window in SQL with single key as session window key") {
    withTempTable { table =>
      val a = spark.sql(
        s"""select session, key, sum(value) as res
           | from $table group by session(time, "10 seconds"), key""".stripMargin)
      checkAnswer(
        spark.sql(
          s"""select session, key, sum(value) as res
             | from $table group by session(time, "10 seconds"), key""".stripMargin)
  ...
  test("session window with single key as session window key") {
    ...
    checkAnswer(
      df.groupBy(session($"time", "10 seconds"), $"key")
  ...
  test("test session window exec with multi group by") {
    ...
    checkAnswer(
      df.groupBy(session($"time", "10 seconds"), $"key1", $"key2")
        .agg(sum($"value").as("res"))
        .orderBy($"session.start".asc, $"key1".asc, $"key2".asc)
        .select($"session.start".cast(StringType),
          $"session.end".cast(StringType), $"key1", $"key2", $"res"),
      Seq(
  ...
  test("test session window exec including multi keys in one partition") {
    withSQLConf("spark.sql.shuffle.partitions" -> "1") {
      ...
      checkAnswer(
        df.groupBy(session($"time", "10 seconds"), $"key")
          .agg(sum($"value").as("res"))
          .orderBy($"session.start".asc, $"key".asc)
          .select($"session.start".cast(StringType),
            $"session.end".cast(StringType), $"key", $"res"),
        Seq(
