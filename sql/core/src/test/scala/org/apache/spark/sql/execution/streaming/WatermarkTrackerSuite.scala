...
class WatermarkTrackerSuite extends StreamTest {
  ...
  test("SPARK-50046: Incorrect watermark advancement") {
    ...
    testStream(union)(
      ...
      Execute { q =>
        ...
        val tracker = WatermarkTracker(spark.conf)
        ...
        def verifyWatermarkMap(expectation: Map[Int, Option[Long]]): Unit = {
          logWarning(s"watermarkMap: ${tracker.watermarkMap}")
          expectation.foreach { case (idx, watermarkValue) =>
            assert(tracker.watermarkMap.get(idx) === watermarkValue,
              s"Watermark value for idx $idx is ${tracker.watermarkMap.get(idx)}, where " +
        ...
        executedPlanFor1stBatch.foreach {
          case e: EventTimeWatermarkExec =>
            logWarning(s"before calling updateWatermark -" +
              s" etw: delay: ${e.delayMs}, ${e.eventTimeStats.value}")
        ...
        // NOTE: This should not advance the watermark hence should be 5, but WatermarkTracker
        // can't track the dropped node, hence it advances the watermark from the remaining nodes,
        // hence min(22, 32) = 22
        // assert(tracker.currentWatermark === 5)
        assert(tracker.currentWatermark === 22000)

        // NOTE: WatermarkTracker updates the map with shifted index. It should only update
        // index 1 and 2, but it updates 0 and 1.
        verifyWatermarkMap(
          Map(
            0 -> Some(22000L),
            1 -> Some(32000L))
        ...
        // NOTE: This does not advance the watermark as expected, but as you see from the above,
        // it's not accounting values of three nodes, but accounting values of "two" nodes. Index
        // 2 should exist in the map, but it does not.
        // assert(tracker.currentWatermark === 5)
        assert(tracker.currentWatermark === 32000)

        // NOTE: WatermarkTracker updates the map with shifted index. It should only update
        // index 1, but it updates 0.
        verifyWatermarkMap(
          Map(
            0 -> Some(52000L),
            1 -> Some(32000L))
