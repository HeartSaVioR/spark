...
class StateSchemaCompatibilityCheckerSuite extends SharedSparkSession {
  ...
  private def verifyException(
    ...
    assert(e.getMessage.contains(newKeySchema))
    assert(e.getMessage.contains(oldKeySchema))

    if (ignoreValueSchema) {
      assert(!e.getMessage.contains(newValueSchema))
      assert(!e.getMessage.contains(oldValueSchema))
    } else {
      assert(e.getMessage.contains(newValueSchema))
      assert(e.getMessage.contains(oldValueSchema))
    }
