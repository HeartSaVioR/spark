...
class CachedInternalKafkaConsumerPoolSuite extends SharedSQLContext {

  test("basic multiple borrows and returns for single key") {
    val pool = CachedInternalKafkaConsumerPool.build
  ...
  private def assertPoolState(pool: CachedInternalKafkaConsumerPool, numIdle: Int,
  ...
  private def assertPoolStateForKey(pool: CachedInternalKafkaConsumerPool, key: CacheKey,
  ...
  private def borrowObjectsPerKey(
                                   pool: CachedInternalKafkaConsumerPool,
  ...
  private def returnObjects(pool: CachedInternalKafkaConsumerPool,
